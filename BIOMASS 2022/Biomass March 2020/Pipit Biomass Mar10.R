...
Pip.PrestoDec <- Pip.Bio20[7:8,1:14]
Pip.DectoAbs <- Pip.Bio20[8:9,1:14]


############ Visualize Plot ##################

Pip.Bio20.Fig <- ggplot(Pip.Bio20, aes(x=Year, y=Mean.Total)) +
  theme_bw()+
  ...
  labs(colour = "Pipit Lake", tag = "b)")+
  geom_line(data = Pip.PrestoDec,
            aes(x=Year,
                y=Mean.Total),
            size=1.5,
            colour="blue")+#lines connecting fish pres to kill
  geom_line(data = Pip.DectoAbs,
            aes(x=Year,
                y=Mean.Total),
            size=1.5,
  ...
  geom_vline(xintercept = 1977, size=1.2, linetype=5)+
